...
void sevenseg(){
	...
	current_time();
	for(i=0;i<=3;i++){
		LPC_GPIO0->FIOPIN|=i<<16;//0.16 TO 0.19 ENABLE
		LPC_GPIO0->FIOPIN|=hexsegment[current[i]]<<4;//0.4 TO 0.11 DATA
	}
}
...
int main(void)
{
	...
	LPC_PINCON->PINSEL0&=0xFFF0000F;//P0.4 TO P0.11 FOR DATA LINE
	LPC_GPIO0->FIODIR|=0xFF<<4;//P0.4 TO P0.11
	LPC_PINCON->PINSEL1&=0XFFFFFF00;//P0.16 TO P0.19 FOR ENABLE
	LPC_PINCON->PINSEL4&=0XFFF00FFF;
